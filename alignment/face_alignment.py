...
class FaceAligner:
    ...
    def align(self, image_rgb, return_metrics=False):
        ...
        if not results.multi_face_landmarks:
            print("Face landmarks not detected for alignment.")
            if return_metrics:
        ...
        # Extract features
        left_eye_pts = [np.array([landmarks.landmark[idx].x * w, landmarks.landmark[idx].y * h]) for idx in self.LEFT_EYE_INDICES]
        right_eye_pts = [np.array([landmarks.landmark[idx].x * w, landmarks.landmark[idx].y * h]) for idx in self.RIGHT_EYE_INDICES]
        nose_pt = np.array([landmarks.landmark[self.NOSE_TIP_INDEX].x * w, landmarks.landmark[self.NOSE_TIP_INDEX].y * h])
